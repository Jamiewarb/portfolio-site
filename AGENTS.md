...
- **Likes feature needs Upstash Redis (optional).** The likes API (`src/pages/api/likes/*`) and `pnpm sync:likes` require Upstash creds. For local dev, create a `.env` (gitignored) with `LIKES_ENABLED=false` so the likes API short-circuits and returns initial data instead of erroring without Redis. The dev server starts fine regardless (the Redis client is created lazily). To exercise likes end-to-end you'd need real `UPSTASH_LIKES_REST_URL` / `UPSTASH_LIKES_REST_TOKEN`.
